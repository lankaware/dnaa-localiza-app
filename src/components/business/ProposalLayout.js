  ...
  useEffect(() => {
    ...
    for (let location of props.list) {
      let line = {
        ...
        dayValue: (location.dayValue || 0)/*.toFixed(2)*/,
        NFValue: (location.dayValue || 0 / 0.825 * 0.175)/*.toFixed(2)*/,
        totalValue: location.dayValue + ((location.dayValue || 0 / 0.825 * 0.175)/*.toFixed(2)*/),
  ...
  return (
    <div ref={ref} style={{ 'marginLeft': '20px' }}>
      ...
      <table style={{ 'width': '80vw', 'marginLeft': '5vw' }} >
        <tbody>
          <tr>
            <td className="layout-header first-header">Nome do Local</td>
